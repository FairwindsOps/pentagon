...
def main():
    ...
    asg_cmd_result = subprocess.check_output(["aws", "autoscaling", "describe-auto-scaling-groups", "--region",  module.params.get('region')])
    asg_result = json.loads(asg_cmd_result)
    results = []
    ...
    if sort:
        results.sort(key=lambda e: e['created'], reverse=(sort_order=='descending'))
    try:
